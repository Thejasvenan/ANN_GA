...
// Function to decode the entire chromosome into weights and thresholds
vector<double> decodeChromosome(const vector<string>& chromosome) {
    vector<double> parameters;

    // Decode weights
    for (int i = 0; i < NUM_WEIGHTS; i++) {
        parameters.push_back(decodeGene(chromosome[i], WEIGHT_MIN, WEIGHT_MAX));
    }

    // Decode thresholds
    for (int i = NUM_WEIGHTS; i < CHROMOSOME_LENGTH; i++) {
        parameters.push_back(decodeGene(chromosome[i], THRESHOLD_MIN, THRESHOLD_MAX));
    }
...
vector<double> feedForward(const vector<double>& input, const Individual& ind) {
    vector<double> decodedParameters = decodeChromosome(ind.parameters);
...
vector<Individual> generateInitialPopulation() {
    ...
    for (int i = 0; i < POPULATION_SIZE; i++) {
		...
		ind.parameters.resize(CHROMOSOME_LENGTH);

        for (int j = 0; j < CHROMOSOME_LENGTH; j++) {
            string gene = "";
            for (int k = 0; k < BITS_PER_GENE; k++) {
                gene += (rand() % 2) ? '1' : '0';  // Randomly generate 0 or 1
            }
            ind.parameters[j] = gene;
        }

        // Fitness is initialized to 0 
        ind.fitness = 0.0;
...
pair<Individual, Individual> crossover(const Individual& parent1, const Individual& parent2) {
    ...
    if (R <= CROSSOVER_RATE) {
		// Pick crossover point between 1 and GENE_SIZE - 1
    ...
    for (int i = 0; i < CHROMOSOME_LENGTH; ++i) {
		for (int j = 0; j < BITS_PER_GENE; ++j) {
			if (((double)rand() / RAND_MAX) <= MUTATION_RATE) {
				// Flip the bit
				ind.parameters[i][j] = (ind.parameters[i][j] == '0') ? '1' : '0';
			}
		}
    }
}
...
int main() {
    ...
    for (int gen = 0; gen < generations; ++gen) {
        ...
        if (gen % 200 == 0 || gen == 1000) { // Log selected generations
            ...
            if (genLog.is_open()) {
                ...
                vector<double> decodedParameters = decodeChromosome(population[0].parameters);
    ...
    if (outputFile.is_open()) {
        vector<double> decodedParameters = decodeChromosome(bestIndividual.parameters);
